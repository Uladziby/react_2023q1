...
import { filterProducts, setSearchTerm, setSelectedProduct } from '../../redux/slices/mainSlice';
...
export const MainPage = () => {
  ...
  const { selectedProduct, filteredProducts, searchTerm } = useSelector(mainSelector);
    ...
    <>
      ...
      {isShowModal && (
        <ModalWindow withBackground onCloseHandler={handlerToggleModal} isShowModal={isShowModal}>
          {selectedProduct ? <DetailInfoModal product={selectedProduct} /> : null}
        </ModalWindow>
      )}
    </>
  );
